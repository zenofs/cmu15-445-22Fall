 ...
 public:
  ...
  bool Insert(const std::string &key, T value) {
    ...
    for (int i = 0; i < key_size - 1; i++) {
      if (!tmp->get()->HasChild(key[i])) {
        tmp = tmp->get()->InsertChildNode(key[i], std::make_unique<TrieNode>(key[i]));
      } else {
    ...
    直接用std::move(TrieNode)作为参数去初始化<TrieNodeWithValue>会出错
    (*parent)->InsertChildNode(key[key_size - 1],
                                    std::move(std::make_unique<TrieNodeWithValue<T>>((std::move(**terminal_node),
    value))));
    ...
    ************/
    ...
    //}
    auto parent = tmp;
    if (!tmp->get()->HasChild(key[key_size - 1])) {
      tmp = tmp->get()->InsertChildNode(key[key_size - 1], std::make_unique<TrieNode>(key[key_size - 1]));
      auto new_node = std::make_unique<TrieNodeWithValue<T>>(std::move(**tmp), value);
      (*parent)->RemoveChildNode(key[key_size - 1]);
      (*parent)->InsertChildNode(key[key_size - 1], std::move(new_node));
      latch_.WUnlock();
      return true;
    }
    tmp = tmp->get()->GetChildNode(key[key_size - 1]);
  ...
  T GetValue(const std::string &key, bool *success) {
    ...
    // Processing the terminal node
    if (!(*tmp)->IsEndNode()) {
      *success = false;
      latch_.RUnlock();
      return {};
    }
    if (auto terminal_node = dynamic_cast<TrieNodeWithValue<T> *>((*tmp).get())) {
      *success = true;
      latch_.RUnlock();
