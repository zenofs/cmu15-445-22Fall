...
// 查找与key相对应的value
INDEX_TEMPLATE_ARGUMENTS
auto B_PLUS_TREE_INTERNAL_PAGE_TYPE::Find(const KeyType &key, const KeyComparator &keyComparator) -> ValueType {
  int l = 1;
  int r = GetSize();
  while (l < r) {
    int mid = (l + r) / 2;
...
/*
 *  非叶子节点容量满了，进行分裂, page_brother为本页的兄弟节点所在页
 */
INDEX_TEMPLATE_ARGUMENTS
